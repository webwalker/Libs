...
/**
 * AQuery internal use only. A shared listener class to reduce the number of
 * classes.
 * 
 */

public class Common implements Comparator<File>, Runnable, OnClickListener,
		OnLongClickListener, OnItemClickListener, OnItemLongClickListener,
		OnScrollListener, OnItemSelectedListener, TextWatcher {
	...
	public Common forward(Object handler, String callback, boolean fallback,
			Class<?>[] sig) {
	...
	private Object invoke(Object... args) {
		...
		if (method != null) {
			...
			Object result = AQUtility.invokeHandler(cbo, method, fallback,
					true, sig, input);
			return result;
		} else if (methodId != 0) {
			...
			case CLEAN_CACHE:
				AQUtility.cleanCache((File) params[0], (Long) params[1],
						(Long) params[2]);
				break;
			case STORE_FILE:
	...
	@Override
	public void onScroll(AbsListView view, int first, int visibleItemCount,
			int totalItemCount) {

		checkScrolledBottom(view, scrollState);

		if (osl != null)
			osl.onScroll(view, first, visibleItemCount, totalItemCount);
	...
	public void onScrollStateChanged(AbsListView view, int scrollState) {
		...
		if (osl != null)
			osl.onScrollStateChanged(view, scrollState);
	}
	...
	private void onScrollStateChanged2(AbsListView lv, int scrollState) {
		...
		if (scrollState == SCROLL_STATE_IDLE) {
			...
			for (int i = 0; i <= count; i++) {
				...
				Number targetPacked = (Number) convertView
						.getTag(AQuery.TAG_NUM);
		...
		if (galleryListen) {
			...
			if (selected != pos) {
				...
				for (int i = 0; i < count; i++) {
					...
					Integer lastDrawn = (Integer) convertView
							.getTag(AQuery.TAG_NUM);
